...
/**
 * Narrator Brain v0.1.0
 * Adapted from Inner Self v1.0.0 by LewdLeah
 * Gives the AI storytelling memory, planning, and plot management capabilities
 */
function NarratorBrain(hook) {
    "use strict";
    /**
    ...
    NARRATOR_ACTIVE_INDICATOR: "🎬"
    // (any text/emoji inside the "" or leave empty)
    ,
    // Symbol shown when narrator is forming a new thought?
    NARRATOR_THINKING_INDICATOR: "💡"
    // (any text/emoji inside the "" or leave empty)
    ,
    // When possible, what percentage of turns should involve an attempt to form a new thought?
    ...
    // (true or false)
    ,
    // Should Narrator Brain model task outputs be displayed inline with the adventure text itself?
    IS_DEBUG_MODE_ENABLED_BY_DEFAULT: false
    // (true or false)
    ,
    }; //——————————————————————————————————————————————————————————————————————————————
     ...
     * @type {Object}
     */
    const NB = state.NarratorBrain = deepMerge(state.NarratorBrain || {}, {
        // Zero-width encoded thought labels for context injection
        encoding: "",
        // Track narrator state (always "Narrator" when active, "" when not)
        agent: "",
        // Monotonically increasing thought label counter
        label: 0,
        // Hash of recent history to detect retry or erase + continue turns
        hash: "",
        // Total number of brain operations performed
        ops: 0,
        // Track if forming a thought this turn (for emoji)
        thinking: false,
        // Auto-Cards integration state (keeping for compatibility)
        AC: {
            enabled: false,
            forced: false,
            event: false
    ...
    if (NB.AC.enabled && (typeof hook === "string") && (hook !== "context") && hasAutoCards()) {
    ...
    class Config {
        ...
        const template = {
            ...
            entry: [
                {
                    message: "Narrator Brain gives the AI storytelling memory and narrative planning capabilities. Edit the settings below to control behavior."
                },
                { message: "Enable Narrator Brain:", ...factory(
                    "allow", S.IS_NARRATOR_ENABLED_BY_DEFAULT
                ) },
                {
                ...
                { message: "Max brain size relative to story context:", ...factory(
                    "percent", S.PERCENTAGE_OF_RECENT_STORY_USED_FOR_NARRATOR,
                    { lower: 1, upper: 95, suffix: "%" }
                ) },
                {
                    message: "Visual indicator when active:",
                    builder: (cfg = {}) => ` "${(
                        config.indicatorActive ?? cfg.setter?.(S.NARRATOR_ACTIVE_INDICATOR)
                    )}"`,
                    setter: (value = null, fallible = false) => (
                        (typeof value === "string")
                        ? (config.indicatorActive = value.replace(/["\u200B-\u200D]+/g, "").trim())
                        : (fallible)
                        ? null
                        : (config.indicatorActive = fallback.indicatorActive)
                    )
                },
                {
                    message: "Visual indicator when thinking:",
                    builder: (cfg = {}) => ` "${(
                        config.indicatorThinking ?? cfg.setter?.(S.NARRATOR_THINKING_INDICATOR)
                    )}"`,
                    setter: (value = null, fallible = false) => (
                        (typeof value === "string")
                        ? (config.indicatorThinking = value.replace(/["\u200B-\u200D]+/g, "").trim())
                        : (fallible)
                        ? null
                        : (config.indicatorThinking = fallback.indicatorThinking)
                    )
                },
            ...
            // Description section contains info
            description: [
                {
                    message: `Narrator Brain ${version} is adapted from Inner Self by LewdLeah. It provides the AI with a persistent brain to track plot threads, manage pacing, plan foreshadowing, and maintain narrative coherence across long adventures.`
                },
                {
                    message: "The Story Director brain card will be created automatically. You can view and edit the narrative priorities in the card's Notes section at any time."
                }
            ]
        };
    ...
    /**
     * Narrator class - Represents the story director brain
     * Stores narrative planning, plot threads, and pacing notes
     * The brain is a key-value store of labeled narrative priorities
     * @class
     */
    class Narrator {
        // Private fields for encapsulation
        // Percentage of context reserved for narrator brain
        #percent;
        ...
        get card() {
            ...
            /**
             * Creates a new brain card for narrator
             ...
             */
            const buildCard = (name = "Story Director") => addStoryCard(
                JSON.stringify({ narrator: true, percentage: this.#percent }),
                (() => {
            ...
            /**
             * Checks if a card is the narrator brain
             * Checks by JSON keys first, then by title
             * @param {Object} card - Card to check
             * @returns {boolean} true if this is the narrator card
             */
            const isNarrator = (card = {}) => {
                // First check: Does it have narrator metadata in keys?
                if ((typeof card.keys === "string") && card.keys.includes("\"narrator\"")) {
                    return true;
                }
                // Second check: Is the title "Story Director"?
                if ((typeof card.title === "string") && (card.title.replace(/^.*\u200B\s*/, "").trim() === "Story Director")) {
                    return true;
                }
                return false;
            };
            
            // Find or create narrator card
            for (const card of storyCards) {
                if (isNarrator(card)) {
                    // Found existing card - ensure it has proper metadata
                    if (!(card.keys && card.keys.includes("\"narrator\""))) {
                        // Pre-existing card without metadata, add it
                        card.keys = JSON.stringify({ narrator: true, percentage: this.#percent });
                    }
                    this.#card = card;
                    return this.#card;
                }
        ...
        /**
         * Gets the narrator's metadata from card
         * Contains configurable settings like context percentage
         * @returns {Object} metadata object with validated percentage
         */
        get metadata() {
            ...
            // Validate and normalize the percentage value
            if (!Number.isInteger(this.#metadata.percentage)) {
                this.#metadata.percentage = (
                    ((typeof this.#metadata.percentage === "number") && Number.isFinite(this.#metadata.percentage))
                    ? Math.min(Math.max(lower, Math.round(this.#metadata.percentage)), upper)
                    : this.#percent
                );
            } else if (this.#metadata.percentage < lower) {
                this.#metadata.percentage = lower;
            } else if (upper < this.#metadata.percentage) {
                this.#metadata.percentage = upper;
            } else {
                return this.#metadata;
            }
        ...
        get brain() {
            ...
            if (!desc.trim().startsWith("{") && !desc.trim().startsWith('"')) {
                ...
                this.#brain = deserializeBrain(desc);
            } else {
                // Old JSON format for backward compatibility
                const source = deserialize(desc, true);
                this.#brain = {};
                for (const key in source) {
    ...
    if ((hook === "context") || Number.isInteger(info.maxChars)) {
        ...
        const unzero = () => ((text = text.replace(/[\u200B-\u200D]+/g, "") || " "), (NB.encoding = ""));
        
        // Narrator is always active (no character-based trigger detection)
        NB.agent = "Narrator";
        ...
        if (config.auto && hasAutoCards()) {
            ...
            NB.AC.enabled = true;
            if (NB.AC.event || (stop === true)) {
                // If AC triggered an event or stop, we're done here
                config.allow ? unzero() : ((NB.encoding = ""), (text ||= " "));
                return;
            }
        } else if (NB.AC.enabled) {
            NB.AC.enabled = false;
        ...
        // No trigger detection needed - narrator is always active
        ...
        // Construct the narrator instance
        const agent = new Narrator({ 
            percent: config.percent,
            indicatorActive: config.indicatorActive || "🎬",
            indicatorThinking: config.indicatorThinking || "💡"
        });
        ...
        if (NB.hash === historyHash()) {
            ...
            const [self, full] = (() => {
                ...
                // Small safeguard: skip if story VERY short (prevents early deletion with pre-seeded brains)
                const storyLength = text.length - text.indexOf(boundary.upper) + boundary.upper.length;
                const constrained = (
                    (800 < joined.length)
                    && (storyLength > 4000)  // Only skip constraint if story < 1.5k (very early)
                    && (((agent.metadata.percentage / 100) * storyLength) < joined.length)
                );
            ...
            /**
             * Occasionally adds a future-planning prompt
             * Keeps the narrator from being too present-focused
...
Follow the format **perfectly**.
        ...
        // Update visual indicator based on whether forming a thought
        for (const card of storyCards) {
            deindicate(card);
        }
        
        const indicator = NB.thinking ? config.indicatorThinking : config.indicatorActive;
        if (indicator && indicator.trim() !== "") {
            const baseTitle = agent.card.title.replace(/^.*\u200B\s*/, "").trim() || "Story Director";
            agent.card.title = `${indicator}\u200B ${baseTitle}`;
        }
        ...
        if (NB.AC.enabled || !/\/\s*A\s*C/i.test(text) || !hasAutoCards()) {
...
(please erase before continuing) <<<
    ...
    // Trim NB.agent name before emptiness check
    if (((NB.agent = NB.agent.trim()) === "") && (blocks.length === 0)) {
    ...
    const agent = new Narrator({ percent: config.percent });
    /**
    ...
    interpreter: for (const block of blocks) {
        ...
        operations.push(() => {
            ...
            NB.encoding = `${(NB.encoding === "") ? "\u200B" : NB.encoding}${(() => {
                let n = NB.label;
                let out = "";
            ...
            text = (text
                .replace(/[\u200B-\u200D]+/g, "")
                .replace(/^\s*/, leadingWhitespace => `${leadingWhitespace}${NB.encoding}`)
            );
            ...
            agent.brain[key] = `${NB.label} → ${thought}`;
    ...
    // We serialize the brain in a UX optimized format (no quotes, no commas, no braces)
    ...
    // Build simple format: key: value\n\n (space after colon, double newline between entries)
    let serialized = "";
    ...
    // Store the serialized brain in the card notes (trim trailing newlines)
    agent.card.description = serialized.trimEnd();
    return;
}
